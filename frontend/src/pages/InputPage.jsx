...
async function geocodeWithMessage(address, label) {
  try {
    ...
    throw new Error(`${label} 주소를 찾을 수 없습니다: ${address}`);
  }
}
...
function InputPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const startGeocode = await geocodeWithMessage(trimmedStart, "출발지");

      const destinationGeocodes = await Promise.all(
        validDestinations.map((address, index) =>
          geocodeWithMessage(address, `배송지 ${index + 1}`)
        )
      );
      ...
      const destinationLocations = destinationGeocodes.map(
        (geocode, index) => ({
      ...
      /*
       * 후보 생성 테스트를 위해 도보 거리와 검색 반경을 넓힌 상태.
       * 테스트 완료 후 원래 기준으로 되돌릴 것.
       */
      const optimizeRequest = {
        ...
        destinations: destinationLocations.map((destination) => ({
          id: destination.id,
      ...
      const routePlan = await optimizeRoute(optimizeRequest);

      console.log("Optimize 요청:", optimizeRequest);
      console.log("Optimize 전체 응답:", routePlan);

      const recommendedOrder = routePlan?.recommended_delivery_order ?? [];

      const findRecommendedStop = (destinationId) => {
        const selectedStopEntry = routePlan?.selected_stops?.find(
          (stop) => stop.destination_id === destinationId
        );

        if (selectedStopEntry) {
          return selectedStopEntry.selected_stop ?? selectedStopEntry;
        }

        const destinationResult = routePlan?.destination_results?.find(
          (result) => result.destination_id === destinationId
        );

        return destinationResult?.recommended_stop ?? null;
      };

      const orderedRouteStops = recommendedOrder
        .map((delivery) => {
          const stop = findRecommendedStop(delivery.destination_id);
          ...
          return {
            id: stop.candidate_id ?? `STOP-${delivery.destination_id}`,
            type: "selected-stop",
            destinationId: delivery.destination_id,
            name: stop.candidate_name ?? stop.name ?? "추천 정차지",
      ...
      const uniqueRouteStops = orderedRouteStops.filter(
        (stop, index, stops) => {
          if (index === 0) {
            return true;
          }
          ...
          return !(
            stop.latitude === previousStop.latitude &&
            stop.longitude === previousStop.longitude
          );
        }
      );
      ...
      const vehicleRouteTargets =
        uniqueRouteStops.length > 0 ? uniqueRouteStops : orderedDestinations;

      const routePoints = [startLocation, ...vehicleRouteTargets];

      console.log("차량 경로 지점:", routePoints);
      ...
      const totalDistanceKm = directionResults.reduce(
        (total, result) => total + Number(result.distance_km ?? 0),
        0
      );

      const totalDurationMinutes = directionResults.reduce(
        (total, result) => total + Number(result.duration_minutes ?? 0),
        0
      );

      const routePath = directionResults.flatMap((result, index) => {
      ...
      navigate("/result", {
        state: {
          ...
          directionsResult,
          routePlan,
          vehicleRouteStops: vehicleRouteTargets,
        },
      });
    } catch (error) {
